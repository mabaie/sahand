...
class AddMagazineForm extends Component {
  constructor(props) {
    ...
    console.log('assinment is :',record.assignment)
    this.state = {
      // data states
      title: record.title,
      date:new Date(record.deadline),
      description: record.description,
      file: record.hasOwnProperty('assignment')?record.assignment.map(ass=>{
				return ass
			}):[],
      //errors
      errors: {
  ...
  componentDidMount() {
    this.calendar = $("#calendar").persianDatepicker({
      ...
      dayPicker: {
        onSelect: date => {
          this.setState({date:new Date(date),changed:true})
        }
      }
    });
  }
    ...
    return e => {
      switch (name) {
        ...
        case "register":
          ...
          // if(this.state.file!=null && (typeof this.state.file)==='object'){
          //   formdata.append("assignment", 
          //     this.state.file
          //   );
          // }
          // formdata.append("title",this.state.title);
          // formdata.append("deadline",this.state.date.toISOString());
          ...
          if (this.checkErrors()) {
            ...
            this.setState({ waiting: true }, () => {
              req
                .put("/update-assignment/" + this.props.record._id, 
                  this.state.file.length>0?{
                    assignment:this.state.file,
                    title:this.state.title,
                    deadline:this.state.date.toISOString(),
    ...
    return (
      <div style={{ padding: 32 }}>
        ...
        >
          ...
          <Grid item className={classes.primary} xs={12}>
            {this.state.file.map((f,i)=>(
              <Grid container spacing={8} alignItems="center">
                <Grid item>
                  <Button onClick={()=>{
                    window.open(configs.publicUrl + f, "_blank")
                  }}>
                    <Typography variant="body1">{persian.toPersian('سند بارگذاری شده '+i)}</Typography>
                  </Button>
                </Grid>
                <Grid item>
          ...
          <Grid item className={classes.primary} xs={12}>
            <ReactFileReader fileTypes={[".pdf",".jpg","image/png",".mp3",".wav",".flac",".aiff",".ogg",".mp4"]} base64={true} multipleFiles={false} handleFiles={this.handleFiles}>
            ...
            <Grid container spacing={8} alignItems="center">
              ...
              <Grid item>
                <TextField
                  ...
                  value = {jMoment(this.state.date.toISOString().substr(0,10),"YYYY-MM-DD").format("jYYYY/jMM/jDD")}
                  id={"calendar"}
