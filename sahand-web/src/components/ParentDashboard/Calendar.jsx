...
class SettingAppointments extends Component {
    ...
    changeDate(y, m) {
        ...
        req.get(`/app/parent/school/events/${localStorage.getItem("student-id")}/${(new Date(date)).valueOf()}`)
        .then(res => { 
    ...
    componentDidMount() {
        ...
        this.setState({ waiting: true }, () => {
        req.get(`/app/parent/school/events/${localStorage.getItem("student-id")}/${(new Date()).valueOf()}`)
        .then(res => { 
        this.findHighlightedDays(res.data)
        var arr = []
        res.data.forEach((el) => {
