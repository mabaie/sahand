...
const validator = Joi.object().keys({
    ...
    Degree: Joi.string().trim().valid('زیر دیپلم', 'دیپلم', 'کارشناسی', 'کارشناسی ارشد', 'دکتری').optional(),
    BirthDay: Joi.string().isoDate().optional(),
    Mobile: Joi.string().regex(/^9[0-9][0-9]{8}$/).optional(),
    Email: Joi.string().email({
        minDomainAtoms: 2
    }).optional(),
    HomeAddress: Joi.string().trim().min(3).max(100)
        .optional(),
    WorkAddress: Joi.string().trim().min(3).max(100)
        .optional(),
    Job: Joi.string().trim().min(3).max(50)
        .regex(persianStringRegExp).optional(),
    HomePhone: Joi.string().regex(/^[1-8][0-9][0-9]{8}$/).optional(),
}).min(1);
